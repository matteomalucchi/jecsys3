...
bool skipZ = false;//false;//true;//false;
bool skipG = false;//false;//true
...
void JERSF_update() {
  ...
  // gROOT->ProcessLine(Form(".! mkdir pdf/%s", version));
  gROOT->ProcessLine(Form(".! mkdir -p pdf/%s/JERSF", version));
  gROOT->ProcessLine(Form(".! mkdir -p pdf/%s/JERSF/vsEta", version));
  ...
  TFile *fout = new TFile(Form("rootfiles/JERSF_%s.root", version),"RECREATE");
  ...
  if (YEAR == "2023"){
    ...
    vsummer_v = {"Summer22-22Sep2023","Summer22EE-22Sep2023","Summer22EE-22Sep2023", "Summer22EE-22Sep2023"};
    vyear_v = {"2022","2022EE","2022EE","2022EE"};
  }
  else{
    vrun_v = {"2022CD","2022E","2022F","2022G","2023Cv123","2023Cv4","2023D"};
    vmc_v = {"Summer22","Summer22EE","Summer22EE","Summer22EE","Summer23","Summer23","Summer23BPIX"};
    vsummer_v = {"Summer22-22Sep2023","Summer22EE-22Sep2023", "Summer22EE-22Sep2023", "Summer22EE-22Sep2023","Summer23-22Sep2023","Summer23-22Sep2023","Summer23BPix-22Sep2023"};
    vyear_v = {"2022","2022EE","2022EE","2022EE","2023","2023","2023"};
   }
  cout << "Selected YEAR: " << YEAR << endl;
  cout << "Selected runs: ";
  for (auto r : vrun_v) cout << r << " ";
  cout << endl;
  ...
  // Grand scan of all eras and recos
    // string vrun[] = {
      /*"2022C_Prompt2022","2022D_Prompt2022", //Pr22
...
// string vmc[] = {
  /*"Summer23MGBPix","Summer23MGBPix", //Pr22
...
//string vmc[] = {"Winter24","Winter24","Winter24","Winter24","Winter24","Winter24"};
  ...
  */
  ...
  else if (TString(cr).Contains("2022") || TString(cr).Contains("2023")) {
    f = new TFile(Form("%s/%s/dijet/jmenano_data_cmb_%s_JME_%s.root",base_path, version, cr, version),"READ");
    fm = new TFile(Form("%s/%s/dijet/jmenano_mc_cmb_%sQCD%s_%s.root", base_path, version, year == "2023" ? "": cyear, run=="2023D" ? "-BPix" : "", version),"READ"); 
    fz = new TFile(Form("%s/%s/zb/jme_bplusZ_merged_%s_%s.root", base_path, version,version, cr),"READ"); 
    fzm = fz;
    fg = new TFile(Form("%s/%s/gam/GamHistosFill_data_%s_%s.root",base_path, version, cr, version),"READ"); 
    fgm = new TFile(Form("%s/%s/gam/GamHistosFill_mc_%sP8%s_%s.root" ,base_path, version, cyear, run=="2023D" ? "-BPix" : "", version),"READ"); 
  ...
  lumi_136TeV = Form("%s - %s - %s",TString(version).Contains("neutrino") ? "PNet incl. neutrinos" : "PNet",cr,cm);
  extraText = "Private";
  ...
  if (yminc!=0 && ymaxc!=1.3)
    c0->SaveAs(Form("pdf/%s/JERSF/JERSF_Eta%1.0f_%1.0f_%s.pdf",version, 10*yminc,10*ymaxc,cr));
  else
    c0->SaveAs(Form("pdf/%s/JERSF/JERSF_Eta13_%s.pdf",version,cr));
  if (run=="2024BCD") c0->SaveAs(Form("pdf/%s/JERSF/JERSF_Eta13_%s.png",version,cr));
  ...
  if (plotEtaBins)
    c1->SaveAs(Form("pdf/%s/JERSF/vsEta/JERSF_eta_%04d_%04d_%s.pdf",
		    version, int(1000.*eta1),int(1000.*eta2),cr));
  ...
  cx0->SaveAs(Form("pdf/%s/JERSF/JERSF_BeforeEta_%s.pdf",version, cr));
  
  cx->SaveAs(Form("pdf/%s/JERSF/JERSF_AllEta_%s.pdf",version, cr));
  ...
  cy->SaveAs(Form("pdf/%s/JERSF/JERSF_Summary_%s.pdf",version, cr));
  ...
  cz->SaveAs(Form("pdf/%s/JERSF/JERSF_Chi2_%s.pdf",version,cr));


  // Produce output text file (FactorizedJetCorrecter style)
  //ofstream ftxt(Form("pdf/JERSF/Summer23_%s_JRV2_MC_SF_AK4PFPuppi.txt",cr));
  //ofstream ftxt(Form("textfiles/Prompt24/Prompt24_%s_JRV1M_MC_SF_AK4PFPuppi.txt",cr));
  //ofstream ftxt(Form("textfiles/Prompt24/Prompt24_%s_JRV2M_MC_SF_AK4PFPuppi.txt",cr));
  //ofstream ftxt(Form("textfiles/Prompt24/Prompt24_%s_JRV3M_MC_SF_AK4PFPuppi.txt",cr));
  //ofstream ftxt(Form("textfiles/Prompt24/Prompt24_%s_JRV4M_MC_SF_AK4PFPuppi.txt",cr));
  //ofstr`eam ftxt(Form("textfiles/Prompt24/Prompt24_%s_JRV5M_MC_SF_AK4PFPuppi.txt",cr));
  //ofstream ftxt(Form("textfiles/Prompt24/Prompt24_%s_JRV5Mc_MC_SF_AK4PFPuppi.txt",cr));
  //ofstream ftxt(Form("textfiles/Prompt24/Prompt24_%s_JRV6M_MC_SF_AK4PFPuppi.txt",cr));
  //ofstream ftxt(Form("textfiles/Prompt24/Prompt24_%s_JRV7M_MC_SF_AK4PFPuppi.txt",cr));
  //ofstream ftxt(Form("textfiles/Prompt24/Prompt24_%s_JRV9M_MC_SF_AK4PFPuppi.txt",cr));
  gROOT->ProcessLine(Form(".! mkdir -p textfiles/%s", version));
  ofstream ftxt(Form("textfiles/%s/%s_%s_JRV1_MC_SF_AK4PFPuppiPNetRegression%s.txt",version, cs, cr,TString(version).Contains("neutrino") ? "PlusNeutrino" : "") );
  cout << "Writing JER SF text file: " << Form("textfiles/%s/%s_%s_JRV1_MC_SF_AK4PFPuppiPNetRegression%s.txt",version, cs, cr,TString(version).Contains("neutrino") ? "PlusNeutrino" : "") << endl;
  // ofstream ftxt(TString(cr).Contains("25") ? Form("textfiles/Prompt25/Prompt25_%s_JRV2M_MC_SF_AK4PFPuppi.txt",cr) : Form("textfiles/ReReco24/ReReco24_%s_JRV9M_MC_SF_AK4PFPuppi.txt",cr));
  ftxt << "{1 JetEta 1 JetPt "
      << "sqrt([0]*fabs([0])/(x*x)+[1]*[1]/x+[2]*[2])/"
      << "sqrt([3]*fabs([3])/(x*x)+[4]*[4]/x+[5]*[5])"
      << " Correction L2Relative}" << endl; // awkward name, but runs
  // Run2 reference header:
  //ftxt << "{1 JetEta 2 JetPt Rho  "
  ...
  for (int i = neta-1; i != -1; --i) {
    ...
    ftxt << Form("%5.3f %5.3f %3d %4d %4d %8.3f %6.4f %7.5f %8.3f %6.4f %7.5f\n",
		-abseta2, -abseta1,
    ...
    // Run2 reference header: 
    //JER &jer = vjer[i];
    //ftxt << Form("%5.3f %6.3f %2d %2d %4d %2d %2d %5.2f %5.3f %6.4f "
    //		" %5.3f %5.3f %5.3f %5.2f\n",
